...
Other optional keyword arguments include `apod`, which specifies an apodization map applied to the data before Fourier transforming (though, by default, no apodization is applied). A simple tapered-border window can be built with `socca.utils.apodization(shape, width)`, e.g. `apod_map = socca.utils.apodization(img.data.shape, width=27)` for a raised-cosine taper 27 pixels wide on every edge. Finally, as in the case of `NormalCorrelated`, the `smooth` and `kernel` arguments control the optional smoothing of the Fourier-space covariance matrix when estimated from the noise realizations. As above, the default is 3 smoothing iterations using a 5-point stencil kernel.
