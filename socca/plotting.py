...
#   --------------------------------------------------------
    def corner(self,name=None,comp=None,fmt='pdf',sigma=10.00,edges=None,
               quantiles=[0.16,0.50,0.84],bins=40,**kwargs):
        ...
        if edges is None:
            if sigma is None:
                ...
                for s in self.fit.samples.T:
                    q = corner.quantile(s,[0.16,0.50,0.84],weights=self.fit.weights)
                    qmin = np.maximum(s.min(),q[1]-sigma*(q[1]-q[0]))
                    qmax = np.minimum(s.max(),q[1]+sigma*(q[2]-q[1]))
                    edges.append([qmin,qmax]); del q, qmin, qmax
                edges = np.array(edges)

        labels = []
        for li, l in enumerate(self.fit.labels):
            if self.fit.units[li] is not None and \
               len(self.fit.units[li])>0:
                labels.append(f'{l}\n[{self.fit.units[li]}]')
            else: labels.append(f'{l}\n')
        labels = np.array(labels)

        indices = np.array([i for i, lbl in enumerate(labels) if any(lbl.startswith(c) for c in comp)])
        
        truths = kwargs.pop('truths',None)
        if truths is not None:
            truths = np.array(truths)[indices]

        corner.corner(data = self.fit.samples[:,indices],
...
#   --------------------------------------------------------
    def comparison(self,name=None,fmt='pdf',fx=1.00,fy=0.38,dpi=72.27*390.00/504.00,
                   cmaps=None,cmap_data=None,cmap_model=None,cmap_residual=None,
        ...
        _gs_kwargs = dict(hspace = 0.475, height_ratios = [1.00,0.05],
                         wspace = 0.245,  width_ratios = [1.00,1.00,1.00],
                         bottom = 0.130, top = 0.995, left = 0.105, right = 1.000-0.105,)
        
        _gs_kwargs.update(gs_kwargs)

        fig = plt.figure(figsize=figsize,constrained_layout=False)
        gs = fig.add_gridspec(2,3,hspace = _gs_kwargs.get('hspace'),
                                  wspace = _gs_kwargs.get('wspace'),
                           height_ratios = _gs_kwargs.get('height_ratios'),
                            width_ratios = _gs_kwargs.get('width_ratios'))
        gs.update(bottom = _gs_kwargs.get('bottom'),
        ...
        if cmaps is not None:
            if isinstance(cmaps, dict):
                ...
                cmap_list[2] = cmaps.get('residuals', cmaps.get('residual', cmap_list[2]))
        ...
        for mi, m in enumerate([imgdata, moddata, imgdata - moddata]):
            ...
            if mi in [0,2]:
                ...
                cbar = matplotlib.colorbar.ColorbarBase(bax,cmap=cmap,norm=norm,orientation='horizontal')
